...
public class Administrator {
    ...
    // FUNCTII DE VERIFICARE EXISTENTA CONT
    ...
    // FUNCTIE COMPLEXA BOLBOACA VALERIA
	...
	if (c == 'c') {
	    ...
	    Concurs concurs = new Concurs(nume, dataDesfasurare, taxa, dataLimita, nrMaxParticipanti, ora, disciplina,
		    sp, salaConcurs);
	...
	if (c == 'w') {
	    ...
	    Workshop w = new Workshop(nume, dataDesfasurare, taxa, dataLimita, nrMaxParticipanti, ora, tematica,
		    salaWorkshop);
    ...
    /*
     * public void creareCont(Persoana p) { // Lista curenta de conturi
     * List<Cont> lista = new ArrayList<Cont>();
     * 
     * // Daca adaugi elevi, foloseste lista de elevi if (p instanceof Elev) {
     * lista = this.conturiElev; } // Daca adaugi cadre didactice, foloseste
     * lista de cadre else { lista = this.conturiCadruDidactic; }
     * 
     * String nume = p.getNume(); String prenumePersoana = p.getPrenume();
     * String[] prenume = prenumePersoana.split(" "); String username = nume +
     * "." + prenume[0]; String parola = "parola"; Cont cont = new
     * Cont(username, parola);
     * 
     * // Verificare daca exista cont de forma nume.prenume for (Cont c : lista)
     * { if (c.getUsername().equals(cont.getUsername())) { int numarPrenume =
     * prenume.length; if (numarPrenume > 1) { username = username + "_" +
     * prenume[1]; cont.setUsername(username); } else { String CNP = p.getCNP();
     * String numbers = CNP.substring(Math.max(0, CNP.length() - 4)); username =
     * username + numbers; cont.setUsername(username); }
     * 
     * } }
     * 
     * lista.add(cont); p.setCont(cont);
     * 
     * }
     */
